...
// =============================================================
// Define Python helper _nnls1_py.
...
file write fh "    cons = {'type': 'eq', 'fun': lambda coef: np.sum(coef)-1}" _n
file write fh "    bounds = [[0.0,1.0] for i in range(xdim)]" _n
file write fh "    fit = minimize(fn,coef0,args=(X, y),method='SLSQP',bounds=bounds,constraints=cons)" _n
...
forvalues r = 1/`R' {
    ...
    python: _nnls1_py("`Y_var'", "Y_hat_ols Y_hat_lasso Y_hat_rf")
    mat w_Y_`r' = beta'
    matrix colnames w_Y_`r' = Y_hat_ols Y_hat_lasso Y_hat_rf
    ...
    python: _nnls1_py("`D_var'", "D_hat_ols D_hat_lasso D_hat_rf")
    mat w_D_`r' = beta'
    matrix colnames w_D_`r' = D_hat_ols D_hat_lasso D_hat_rf
