#!/usr/bin/env python3
"""Extract `@snippet:NAME ... @end` blocks from the _GN sources/logs.

Walks _GN/{R,Python,Stata}, skips any file with "byhand" in its name,
extracts each marked section, and writes it to
`_includes/GN/<name>_<lang>.txt` where <lang> is r / python / stata.
...
SRC_DIRS = {
    "r":      ROOT / "_GN" / "R",
    "python": ROOT / "_GN" / "Python",
    "stata":  ROOT / "_GN" / "Stata",
}
OUT_DIR = ROOT / "_includes" / "GN"
...
def main() -> int:
    ...
    for lang, src_dir in SRC_DIRS.items():
        ...
        for path in sorted(src_dir.iterdir()):
            if "byhand" in path.name.lower() or not path.is_file():
                continue
